...
function request(path, options = {}) {
  const headers = options.body
    ...
    : options.headers;
  return fetch(path, { credentials: "include", ...options, headers }).then(parseResponse);
}
...
export function createHttpHostedApi() {
  return {
    ...
    signup: (input) => request("/api/auth/signup", { method: "POST", body: JSON.stringify(input) }),
    login: (input) => request("/api/auth/login", { method: "POST", body: JSON.stringify(input) }),
    forgotPassword: (input) => request("/api/auth/forgot-password", { method: "POST", body: JSON.stringify(input) }),
    getPreferences: () => request("/api/preferences"),
    updatePreferences: (input) => request("/api/preferences", { method: "PUT", body: JSON.stringify(input) }),
    getWatchlist: () => request("/api/watchlist"),
    updateWatchlist: (input) => request("/api/watchlist", { method: "PUT", body: JSON.stringify(input) }),
    getMatches: () => request("/api/matches"),
    requestCompany: (input) => request("/api/company-requests", { method: "POST", body: JSON.stringify(input) }),
  };
}

const clone = (value) => JSON.parse(JSON.stringify(value));

export function createMockHostedApi({ latency = 0, failures = {}, fixtures } = {}) {
  const state = clone(fixtures || makeHostedFixtures());

  const respond = (key, value) => new Promise((resolve, reject) => {
    const finish = () => {
  ...
  return {
    ...
    getMe: () => respond("me", { ...state.me, last_successful_scan_at: state.last_successful_scan_at }),
    signup: (input) => respond("signup", { user: { ...state.me, email: input.email, email_verified: false } }),
    ...
    requestCompany: (input) => respond("requestCompany", { id: "request-demo", status: "received", ...input }),
  };
}

export const hostedApi = import.meta.env.VITE_HOSTED_API_MODE === "live"
  ? createHttpHostedApi()
  : createMockHostedApi();
