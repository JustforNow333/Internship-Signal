      ...
      <div className="page-heading dashboard-heading">
        <div>
          ...
          <p>
            ...
            so the freshest openings stay first.
          </p>
        </div>
        <button
      ...
      <section className="dashboard-stats" aria-label="Watchlist summary">
        <article>
          ...
          <div>
            ...
            <span>Companies watched</span>
          </div>
          <button onClick={() => navigate("/app/watchlist")}>Manage</button>
        </article>
        <article>
          <span className="stat-icon" aria-hidden="true">
            ⌁
          ...
          <button onClick={() => navigate("/app/settings")}>Edit</button>
        </article>
        <article>
          <span
            className={`stat-icon ${alertsPaused ? "paused" : "active"}`}
            aria-hidden="true"
          >
            ◉
          </span>
          <div>
            <strong>{alertsPaused ? "Paused" : "Active"}</strong>
            <span>{alertFrequencyLabel(data.preferences.alert_frequency)}</span>
          </div>
        ...
        <article>
          ...
          <div>
            <strong>{formatScanTime(data.me.last_successful_scan_at)}</strong>
            <span>Last successful scan</span>
          </div>
          <small>Scheduled scans</small>
        </article>
      </section>
        ...
        <aside className="dashboard-sidebar">
          <section className="side-card">
            <div className="side-card-head">
              <p className="eyebrow">Your role signal</p>
              <button onClick={() => navigate("/app/settings")}>Edit</button>
            </div>
          ...
          <section className="side-card">
            ...
            <p className="side-description">
              Application links stay prominent so you can move quickly.
            </p>
            <EmailPreview match={matches[0]} />
          </section>
        </aside>
      </section>
      <MatchDrawer
