...
dataclasses in `models.py`, and YAML loading plus watchlist validation still in
the transitional `_legacy.py`.
...
written in `.env` reaches it. Process environment values still win over `.env`.
