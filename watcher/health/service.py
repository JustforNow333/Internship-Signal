"""Health-alert orchestration and the SMTP boundary.

``evaluate_and_send_health_alerts`` is the only entry point the pipeline calls.
It records every candidate, delivers the HIGH ones immediately, and defers the
rest to one daily digest. Email failure is logged and reported, never raised:
source-health mail must not affect internship state or run success.
"""
...
# Pinned rather than derived from ``__name__``: the console format prints the
# logger name, so operators already grep production runs for this exact one.
