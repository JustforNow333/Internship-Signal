"""The two SQLite health stores.

``SourceHealthStore`` lives beside the durable seen store and holds per-source
attempts and current state. ``HealthAlertStore`` holds alert cooldowns, alert
events, digest ledgers, and coverage snapshots. Both create their tables on
open and migrate additively, so an existing production database keeps working.
"""
