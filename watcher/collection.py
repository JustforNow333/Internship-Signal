"""Live collection: adapter resolution, fetch planning, outcomes, and counters.

This module owns everything between the watchlist and a replayable
:class:`~watcher.collection_snapshot.CollectionBatch`: which direct adapter
serves a company, how direct and GitHub backstop fetches are planned and
executed under the active collection mode, how each outcome becomes rows,
errors, and source attempts, and the Workday transport counters summarized
from those attempts.
"""
...
def _default_direct_sources(
    *,
    ...
    """Build the production adapter set from the canonical source registry.

    Kept as a named seam so collection code and tests have one patchable entry
    point for default adapter construction. Register new direct sources in
    `watcher/sources/registry.py`, never here.
    """
