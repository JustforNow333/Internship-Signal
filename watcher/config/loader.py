...
Validation is deliberately not here yet. The per-ATS rules, hostname and feed
URL checks, uniqueness checks, and the accepted-value vocabularies still live
in `_legacy.py`, and this module calls them. That temporary dependency goes
away when validation is extracted next.
"""
...
# Validation still owned by the transitional module; the next commit moves it.
