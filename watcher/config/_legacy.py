"""Transitional home for the watcher's configuration validation rules.

Only validation is left here: the accepted-value vocabularies, the per-ATS
entry rules, hostname and feed-URL checks, and the uniqueness checks. The data
models moved to `models.py`, the process-environment layer to `env.py`, and
watchlist loading to `loader.py`, which calls the rules below. The next commit
moves them into `validation.py` and retires this module.
"""
