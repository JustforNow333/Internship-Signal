...
environment lives here: dotenv loading, the ``WATCHER_*`` settings, the
bounded coercion helpers those settings share, and their defaults. YAML
loading lives in ``loader.py``; watchlist validation lives in
``validation.py``.

This is the lowest module in the config package. It imports nothing else from
the package at module scope because ``models.py`` reads the environment-derived
``DEFAULT_SEEN_DB_PATH`` while its class bodies execute. Functions that need a
model constant use deferred imports.

Import-time behavior is deliberate: ``load_dotenv()`` runs before
``DEFAULT_SEEN_DB_PATH`` is evaluated, and process values retain precedence
because dotenv loading uses ``os.environ.setdefault``.
"""
...
def load_dotenv(path: str | Path = DEFAULT_DOTENV_PATH) -> None:
    ...
    Supports normal dotenv lines (``KEY=value``) and PowerShell assignments
    (``$env:KEY = "value"``). Existing process values are left alone.
    """
...
def resolve_analysis_cache_path(
    ...
    raw = os.getenv("WATCHER_ANALYSIS_CACHE_PATH") if value is None else value
...
def analysis_cache_enabled(value: str | bool | None = None) -> bool:
    ...
    raw = os.getenv("WATCHER_ANALYSIS_CACHE_ENABLED") if value is None else value
    if raw is None:
