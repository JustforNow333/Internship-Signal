...
from watcher.sources.epic import DEFAULT_MAX_PAGE_BYTES, DEFAULT_MAX_SEARCH_BYTES
...
@pytest.mark.parametrize("posting_id", ["", "0", "-1", "abc", True])
def test_invalid_native_ids_fail_closed(posting_id):
    source = source_for(
        next_page([{"id": posting_id}], {}),
        [{"id": posting_id}],
    )

    with pytest.raises(SourceSchemaError, match="posting ID is invalid"):
        source.fetch(company())
...
def test_transport_helpers_keep_separate_bounded_response_limits(monkeypatch):
    calls: list[tuple[str, int]] = []

    def request_text(_url, _source_name, **kwargs):
        calls.append(("page", kwargs["max_response_bytes"]))
        return next_page([], {})

    def request_json(_url, _source_name, **kwargs):
        calls.append(("search", kwargs["max_response_bytes"]))
        return []

    monkeypatch.setattr("watcher.sources.epic.get_text_response", request_text)
    monkeypatch.setattr("watcher.sources.epic.get_json_response", request_json)

    assert EpicSource().fetch(company()) == []
    assert calls == [
        ("page", DEFAULT_MAX_PAGE_BYTES),
        ("search", DEFAULT_MAX_SEARCH_BYTES),
    ]
...
def test_cached_search_request_uses_the_same_bounded_retry_contract():
    calls = 0
    delays: list[float] = []
    ...
    source = EpicSource(
        request_text=lambda *_: next_page([], {}),
...
def test_schema_contract_failure_is_not_retried_or_followed_by_search():
    page_calls = 0
    search_calls = 0
    ...
    def request_text(*_):
        nonlocal page_calls
        page_calls += 1
        return "<html>not the official Flight contract</html>"
