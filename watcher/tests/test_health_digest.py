"""Deferred MEDIUM/INFO reporting: digest collapsing, windows, and wording.

Small record-level noise and recovered retries stay degraded and visible, and
are reported by the same daily digest that carries every other MEDIUM and INFO
incident. These tests cover that digest end to end - deferral, collapsing,
occurrence counts, the reporting window and its catch-up clamp - plus the
immediate alert rendering that must keep sensitive values out of email.
"""
...
from watcher.tests.health_alert_helpers import (
    ...
    _evaluate,
    _evaluate_states,
    _healthy_after_minor,
    _minor_recovery_transition,
    _minor_state,
    _other_state,
    _retry_state,
    _state,
