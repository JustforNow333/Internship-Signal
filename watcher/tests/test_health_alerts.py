...
def test_a_first_failure_without_repeat_history_stays_high():
    """Nothing recurring to compare against, so the failure interrupts."""
...
def test_unhealthy_github_feed_posture_keeps_a_repeated_failure_high():
    """A stale feed removes the usable posture the prior repeats were judged on."""

    candidate = _failure_candidate(
        github_rows=5,
        feeds=(_feed_state(last_nonzero=NOW - timedelta(days=30)),),
        history=_prior_occurrences(
            FLAP_REPEAT_THRESHOLD * 2,
...
def test_legacy_events_without_an_error_kind_are_not_evidence(tmp_path):
    """Payloads predating the stored error kind can never prove a repeat."""

    db = tmp_path / "state.sqlite"
    legacy = replace(_failure_candidate(github_rows=0), error_kind=None)
    with HealthAlertStore(db) as store:
        for index in range(FLAP_REPEAT_THRESHOLD * 2):
            store.record_detected(
                replace(legacy, run_id=f"legacy-{index}"),
