"""Console report, application heartbeat, and the sanitized health JSON report.

Everything here reads a finished :class:`~watcher.pipeline.RunResult` and
writes it out. The heartbeat is a single line whose field names and order are
consumed by the scheduled workflow, so both are treated as output contracts.
"""
...
from watcher.pipeline import SHADOW_GENERATION_REPORT_LIMIT, RunResult
