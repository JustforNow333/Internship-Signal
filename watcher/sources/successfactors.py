...
class SuccessFactorsSource(DirectDiagnosticsMixin):
    ...
    def __init__(
        ...
        *,
        ...
        self._crawl_request_attempts = 0
        self._crawl_retry_attempts = 0
    ...
    def request_attempts(self) -> int:
        return self._completed_request_attempts + self._crawl_request_attempts

    @property
    def retry_attempts(self) -> int:
        return self._completed_retry_attempts + self._crawl_retry_attempts
    ...
    def _archive_crawl_request_counts(self) -> None:
        self._completed_request_attempts += self._crawl_request_attempts
        self._completed_retry_attempts += self._crawl_retry_attempts
    ...
    def _request_page(self, url: str) -> str:
        ...
        for attempt in range(1, self._max_attempts + 1):
            self._crawl_request_attempts += 1
            try:
                ...
                # Retrying this page reuses the identical startrow. Exhausting
                # either the page limit or this crawl's budget fails closed.
                if (
                    not exc.retryable
                    or attempt == self._max_attempts
                    or self._crawl_retry_attempts >= self._max_crawl_retries
                ):
                    raise
                self._crawl_retry_attempts += 1
